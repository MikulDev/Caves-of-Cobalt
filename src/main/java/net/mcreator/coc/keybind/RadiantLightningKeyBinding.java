...
package net.mcreator.coc.keybind;
...
import net.mcreator.coc.procedures.RadiantLightningTriggerProcedure;
...
public class RadiantLightningKeyBinding extends CocModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					CocMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
		...
		if (type == 0) {
			{
				...
				RadiantLightningTriggerProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
