...
import net.mcreator.coc.procedures.MalaDashWUnpressProcedure;
...
public class MalaDashWKeyBinding extends CocModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					...
					CocMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(1, dt));
		...
		if (type == 1) {
			{
				...
				MalaDashWUnpressProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
