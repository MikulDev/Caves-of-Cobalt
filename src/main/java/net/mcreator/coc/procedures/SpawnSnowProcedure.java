	...
	public static void executeProcedure(Map<String, Object> dependencies) {
		if (dependencies.get("entity") == null) {
			if (!dependencies.containsKey("entity"))
				CocMod.LOGGER.warn("Failed to load dependency entity for procedure SpawnSnow!");
			return;
		}
		if (dependencies.get("x") == null) {
			if (!dependencies.containsKey("x"))
				CocMod.LOGGER.warn("Failed to load dependency x for procedure SpawnSnow!");
			return;
		}
		if (dependencies.get("y") == null) {
			if (!dependencies.containsKey("y"))
				CocMod.LOGGER.warn("Failed to load dependency y for procedure SpawnSnow!");
			return;
		}
		if (dependencies.get("z") == null) {
			if (!dependencies.containsKey("z"))
				CocMod.LOGGER.warn("Failed to load dependency z for procedure SpawnSnow!");
			return;
		}
		if (dependencies.get("world") == null) {
			if (!dependencies.containsKey("world"))
				CocMod.LOGGER.warn("Failed to load dependency world for procedure SpawnSnow!");
			return;
		}
		...
		if ((((world.getBiome(new BlockPos((int) x, (int) y, (int) z)).getTemperature(new BlockPos((int) x, (int) y, (int) z)) * 100.f) < 0.15)
			...
			for (int index0 = 0; index0 < (int) ((Math.round((Math.random() * 15)) + 15)); index0++) {
				...
				world.setBlockState(new BlockPos((int) (entity.getPersistentData().getDouble("randX")),
						...
						IcenatorBlock.block.getDefaultState(), 3);
			}
			...
			if (!world.getWorld().isRemote && world.getWorld().getServer() != null) {
				world.getWorld().getServer().getCommandManager().handleCommand(
						...
						"fill ~10 ~10 ~10 ~-10 ~-10 ~-10 caves_of_cobalt:coldbiomeblock 0 replace stone");
			}
			if (!world.getWorld().isRemote) {
				Template template = ((ServerWorld) world.getWorld()).getSaveHandler().getStructureTemplateManager()
						.getTemplateDefaulted(new ResourceLocation("coc", "permafrost_heart"));
				if (template != null) {
					template.addBlocksToWorld(world, new BlockPos((int) x, (int) (y - 3), (int) z),
							new PlacementSettings().setRotation(Rotation.NONE).setMirror(Mirror.NONE).setChunk(null).setIgnoreEntities(false));
				}
			}
			...
			if (!world.getWorld().isRemote) {
				...
				if (_tileEntity != null)
					_tileEntity.getTileData().putDouble("biomeID", 1);
